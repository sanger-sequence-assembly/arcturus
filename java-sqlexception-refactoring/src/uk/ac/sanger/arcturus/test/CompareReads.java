...
public class CompareReads {
	...
	public CompareReads(String[] args) {
		...
		if (instance1 == null || organism1 == null || instance2 == null
				|| organism2 == null) {
				...
				if (read1 != null && read2 != null) {
					...
					if (read2 == null)
						System.out.println("*** Read " + readname
								+ " not present in " + instance2 + "/"
								+ organism2);

					if (read1 == null)
						System.err.println("*** Read " + readname
								+ " not present in primary database");
				}
			}
	...
	private void compareReads(Read read1, Read read2, PrintStream ps)
			throws SQLException {
		...
		if (chemistry1 != chemistry2)
			reportMismatch(ps, "Chemistry mismatch: " + chemistry1 + " vs "
					+ chemistry2);
	...
	private void compareTemplates(Template template1, Template template2,
		...
		if (!tname1.equalsIgnoreCase(tname2))
			reportMismatch(ps, "Template name mismatch: " + tname1 + " vs "
					+ tname2);
	...
	private void compareLigations(Ligation ligation1, Ligation ligation2,
			PrintStream ps) {
		...
		if (!lname1.equalsIgnoreCase(lname2))
			reportMismatch(ps, "Ligation name mismatch: " + lname1 + " vs "
					+ lname2);
		...
		if (sihigh1 != sihigh2)
			reportMismatch(ps, "Upper insert size mismatch: " + sihigh1
					+ " vs " + sihigh2);
		...
		if (silow1 != silow2)
			reportMismatch(ps, "Lower insert size mismatch: " + silow1 + " vs "
					+ silow2);
	...
	private void compareClones(Clone clone1, Clone clone2, PrintStream ps) {
		...
		if (!cname1.equalsIgnoreCase(cname2))
			reportMismatch(ps, "Clone name mismatch: " + cname1 + " vs "
					+ cname2);
	}
	...
	private void compareClipping(Clipping clip1, Clipping clip2, String type,
			PrintStream ps) {
		if (clip1 == null)
			reportMismatch(ps, type + " clipping missing from primary sequence");
		else if (clip2 == null)
			reportMismatch(ps, type
					+ " clipping missing from secondary sequence");
		else if (!clip1.equals(clip2))
			reportMismatch(ps, type + " clipping mismatch: " + clip1 + " vs "
					+ clip2);
	}
