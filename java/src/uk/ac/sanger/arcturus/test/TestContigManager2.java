		...
		try {
			...
			while (true) {
				...
				if (verb.equalsIgnoreCase("preload")) {
					if (words.length < 2) {
						System.out
								.println("The preload command requires one or more objects");
					} else {
					...
					if (words.length < 2) {
						System.out
								.println("The contig command requires at least a contig ID");
					} else {
						...
						if (contig_id > 0) {
							...
							Contig contig = adb
									.getContigByID(contig_id, option);
							...
							System.out.println("Invalid contig ID: \""
									+ words[1] + "\"");
						}
