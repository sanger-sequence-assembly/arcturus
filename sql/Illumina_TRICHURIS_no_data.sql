...
DROP TABLE IF EXISTS `OLD_PROJECT_DIRECTORY`;
...
/*!50001 DROP TABLE IF EXISTS `CURRENTCONTIGS`*/;
/*!50001 DROP VIEW IF EXISTS `CURRENTCONTIGS`*/;
...
/*!50001 DROP TABLE IF EXISTS `DUPLICATEREADS`*/;
/*!50001 DROP VIEW IF EXISTS `DUPLICATEREADS`*/;
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`arcturus_dba`@`%` SQL SECURITY INVOKER */
/*!50001 VIEW `DUPLICATEREADS` AS select `SEQ2READ`.`read_id` AS `read_id`,count(0) AS `hits` from (`CURRENTCONTIGS` left join (`MAPPING` left join `SEQ2READ` on((`MAPPING`.`seq_id` = `SEQ2READ`.`seq_id`))) on((`CURRENTCONTIGS`.`contig_id` = `MAPPING`.`contig_id`))) group by `SEQ2READ`.`read_id` having (`hits` > 1) */;
