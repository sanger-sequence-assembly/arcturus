...
# test if there is a project BIN or (small)bin in assembly 1
...
# no, it does not exist; test for existence of 'smallbin' project
        ($project,$msg) = $this->getProject(assembly_id=>1,
                                            projectname=>'smallbin',
                                            binautoload=>0);
        return ($project,$msg) if $project;
# also this one doesn't exist, create it      
        &insertProject($dbh,1, 1,'smallbin',$this->getArcturusUser(),
                            'arcturus','autogenerated',undef,undef);
        return $this->getProject(assembly_id=>1,
                                 projectname=>'smallbin',
                                 binautoload=>0);
    }
    else {
...
# to query (one cannot enter project 0)           )                     )
...
# public method: add a new project to the database
    ...
    my @data   = ($project->getAssemblyID() || 0, # 0 will fail on INNODB, hence must specify
...
# test current status (override with option 'force')
    ...
    return $this->assignContigIDsToProjectID([($contig_id)],$project_id,%options);
}
...
# public method for allocating several contigs to a project
    ...
    my $logger = $this->verifyLogger("assignContigsToProject");

    my @contigids;
    foreach my $contig (@$contigs) {
        &verifyParameter($contig,'assignContigsToProject','Contig');
        my $contig_id = $contig->getContigID();
        next unless $contig_id;
	push @contigids, $contig->getContigID();
    }
    
    unless (scalar(@$contigs) && scalar(@$contigs) == scalar(@contigids)) {        
        return 0,"There are missing or invalid contig IDs";
    }

    my $project_id = $project->getProjectID() || return (0,"Missing project ID");

    return $this->assignContigIDsToProjectID([@contigids],$project_id,%options);
}
...
# assign directly
    ...
    unless ($this->userCanMoveAnyContig($user)) {
        return 0,"user $user cannot move contigs";
    }

    unless ($this->userCanLockProject($user)) {
        return 0,"user $user has no locking privilege on project nr $pid";
    }
...
# (to acquire a lock requires ownership or overriding privilege): test lockowner
    elsif (!&setLockedStatus($dbh,$project_id,$user)) {
...
# return if all expected entries have been updated
    ...
    if ($nrow == scalar(@$contig_ids)) {
        &setUnLockedStatus($dbh,$project_id,$user) unless $donotreleaselock;
        return (1,$message);
    }
...
# the user has privilege and can (try to) lock the project
            ...
            my $islocked = &setLockedStatus($dbh,$pid,$user); # acquire
...
# ok, the project is found to be locked and owned by the current user
    ...
    if (&setUnLockedStatus($dbh,$pid,$lockowner)) {
...
sub oldsetLockedStatus { # DEPRECATED
# private function
    my $dbh = shift;
...
#   my @roleswithlockprivilege = ('superuser','administrator',
#                                 'teamleader','coordinator','finisher');
#   my $roleswithlockprivilege # join "','",@roleswithlockprivilege;

# put a lock on a project if it is not already locked

#    my $query = "update PROJECT,USER"
    my $query = "update PROJECT"
              . "   set lockdate = now(), lockowner = ? "
              . " where project_id = ? "
              . "   and lockdate is null"
              ...
              . "   and status not in ('finished','quality checked')";
#              . "   and USER.username = ?"
#              . "   and USER.role in ('$roleswithlockprivilege')";

    my @qdata = ($user,$projectid);
#    my @qdata = ($user,$projectid,$user);
...
sub setUnLockedStatus {
# private function
    my $dbh = shift;
    my $projectid = shift || return undef;
    my $user = shift;

    &verifyPrivate($dbh,"setUnLockedStatus");
