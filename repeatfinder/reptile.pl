...
unless (defined($mode)) {
    &showUsage("No mode specified for the output (SAM or SQL)");
    exit 1;
}
    ...
    if ($line =~ /^\s*\d+\s+\d+\.\d+\s+\S+\s+\S+\s+\d+\s+\d+\s+\d+\s+\S+\s+\S+\s+\d+\s+\d+\s+\d+/) {
    	 ...
    	 = split(/\s+/,$line, 13);
			...
			if ($contig_id == 0) {
					...
					#Gap5 example "*       768     00016.contig03989       48      255     1M      *       0       0       *       *       CT:Z:+;COMM;Note=5 As at pos 49\n" 
					print STDOUT "*       768     $name1       $start1      $len2     1M      *       0       0       *       *       CT:Z:+;REPT;Note=$name2 auto-generated by REPTILE\n" if ($mode eq "SAM");
			}
...
print STDERR "$not_found SAMTAGs cannot be generated because the contig id cannot be found\n";
print STDERR"$invalid SAMTAGs cannot be generated because the matches file line was invalid\n";
$adb->disconnect();
