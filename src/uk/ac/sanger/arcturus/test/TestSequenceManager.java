...
public class TestSequenceManager {
	...
	public static void main(String args[]) {
		...
		if (args.length < 3) {
			System.out
					.println("Argument(s) missing: instance organism readrange(s)");
			System.exit(1);
		}
			...
			for (int i = 0; i < readranges.length; i++) {
				...
				try {
					for (int id = firstid; id <= lastid; id++) {
						...
						Sequence sequence = fullSequence ? adb
								.getFullSequenceBySequenceID(id) : adb
								.getSequenceBySequenceID(id);
