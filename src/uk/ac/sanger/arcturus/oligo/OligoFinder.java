...
public class OligoFinder {
	...
	public int findMatches(Oligo[] oligos, Contig[] contigs) throws SQLException {
		int found = 0;
	...
	private int findMatches(Oligo[] oligos, Contig contig) throws SQLException {
		...
		if (listener != null) {
			event.setEvent(OligoFinderEvent.START_CONTIG, null, contig, 0, false);
			listener.oligoFinderUpdate(event);
		}
	...
	private Contig[] getContigsForProjects(Project[] projects) throws SQLException {
	...
	private byte[] inflate(byte[] cdata, int length) {
		...
		try {
			...
			Arcturus.logWarning("An error occurred whilst decompressing consensus data", dfe);
			data = null;
		}
