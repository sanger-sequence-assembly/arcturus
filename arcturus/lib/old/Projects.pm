...
# my $PROJECTS; # table handle for PROJECTS table
...
sub init {
# initialise by defining the PROJECTS table handle
    my $prototype = shift;
...
#    $PROJECTS = $tblhandle->spawn('PROJECTS',$database);
...
# here we need to inherit from Assembly, which therefore must exist
        die "Missing Assembly reference" unless $Assembly;
...
# okay, we seem to have everything to build a new instance
    ...
    my $self = $class->SUPER::new($Assembly->tableHandle);
