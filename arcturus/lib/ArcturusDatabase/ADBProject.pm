...
# to exist in order to return a project
    ...
    if ($binautoload && ($usecontigid || $lookingforbin)) {
        my $project = $this->getProject(assembly_id=>0,projectname=>'BIN');
        undef my $trialname; $trialname = 'bin' if $project;
        &createBinProject($dbh,$this->getArcturusUser(),$trialname);
