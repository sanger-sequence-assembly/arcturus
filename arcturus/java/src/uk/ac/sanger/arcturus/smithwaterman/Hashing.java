...
public class Hashing {
	...
	if (endsize < 0) {
	    System.err.println("End size must be greater than or equal to zero");
	    System.exit(1);
	}
	...
	while (true) {
		...
		calculateHashes(dna.substring(seqlen - endsize, seqlen), seqlen - endsize, seq_id);
	    } else
		calculateHashes(dna, 0, seq_id);
	}

	System.out.println("Hash vector contains " + hashes.size() + " entries");

	reportMemory(System.out);

	HashEntryComparator comp = new HashEntryComparator(HashEntryComparator.BY_HASH_SEQID_POS);
	...
	ps.println("MEMORY: Total/free/used=" + format.format(total) + "/" + format.format(free) +
		   "/" + format.format(used));
    }
    ...
    class HashEntryComparator implements Comparator {
	...
	public int compare(Object o1, Object o2) {
	    HashEntry entry1 = (HashEntry)o1;
	    HashEntry entry2 = (HashEntry)o2;
