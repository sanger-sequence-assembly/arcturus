...
public class TestSmithWaterman extends JFrame implements ActionListener {
    ...
    private JTextField txtBandwidth = new JTextField("3", 4);
	...
	try {
	    ...
	    int bandwidth = Integer.parseInt(txtBandwidth.getText());

	    String sequenceA = txtSequenceA.getText().replaceAll("\n","");
	    String sequenceB = txtSequenceB.getText().replaceAll("\n","");

	    swPanel.display(sequenceA, sequenceB, sMatch, sMismatch, sGapInit, sGapExt, banded, bandwidth);
	}
	catch (Exception e) {
	    e.printStackTrace();
	}
