...
public class ScaffoldMoveContigs {
	...
	private void analyseScaffold() {
	    ...
	    System.out.println("### SCAFFOLD " + scaffoldID + "\n# Biggest is " + biggest + " with " + maxsum + "bp");

	    for (int i = 0; i < projects.length; i++) {
		if (i != biggest && sumlength[i] > 0) {
		    System.out.println("# Project " + i + " : " + projects[i].size() + " contigs " +
				       sumlength[i] + " bp");
		    for (Iterator iterator = projects[i].iterator(); iterator.hasNext();) {
			Contig contig = (Contig)iterator.next();
			System.out.println(contig.getID() + " " + i + " " + biggest + " " + contig.getSize());
		    }
		}
	    }
	}
