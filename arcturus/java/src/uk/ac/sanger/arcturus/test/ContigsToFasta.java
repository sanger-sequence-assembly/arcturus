...
public class ContigsToFasta {
	...
	PrintStream psFasta = (fastafilename != null) ? new PrintStream(new FileOutputStream(fastafilename)) : System.out;
	PrintStream psQuality = (qualityfilename != null) ? new PrintStream(new FileOutputStream(qualityfilename)) : null;
	...
	try {
		...
		Bounds bounds = doClipping ? calculateBounds(quality, threshold) : new Bounds(0, dna.length);

		writeDNAAndQuality(contig.getName(), dna, quality, bounds, psFasta, psQuality, depad);
	    }
	...
	String caption = ">" + seqname + " " + dna.length + " " + bounds.left + ".." + bounds.right;
    ...
    private void writeDNA(byte[] dna, Bounds bounds, PrintStream ps) {
	for (int i = bounds.left; i < bounds.right; i += 50) {
	    int sublen = (i + 50 < bounds.right) ? 50 : bounds.right - i;
	    ps.write(dna, i, sublen);
	    ps.print('\n');
	}
    }

    private void writeQuality(byte[] quality, Bounds bounds, PrintStream ps) {
	StringBuffer buffer = new StringBuffer();

	int len = bounds.right - bounds.left;

	for (int i = 0; i < len; i++) {
	    int qual = (int)quality[bounds.left + i];
	    buffer.append(qual);
