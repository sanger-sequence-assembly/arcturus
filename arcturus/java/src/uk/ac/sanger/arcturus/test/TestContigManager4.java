...
public class TestContigManager4 {
	...
	try {
		...
		for (int i = 0; i < words.length; i++) {
			...
			if (contig != null) {
			    System.out.println("Contig id=" + contig.getID() + ", length=" + contig.getLength() +
				...
				for (int imap = 0; imap < mappings.length; imap++) {
				    ...
				    System.out.println("Mapping " + imap + " : seq_id=" + sequence.getID() +
						       ", cstart=" + mapping.getContigStart() +
						       ", cfinish=" + mapping.getContigFinish() +
						       ", sense=" + (forward ? "forward" : "reverse") +
						       ", read=" + read.getName());
					...
					for (int iseg = 0; iseg < segments.length; iseg++) {
					    ...
					    System.err.println("    " + cstart + " " + cfinish + " --> " + rstart + " " + rfinish);
					}
