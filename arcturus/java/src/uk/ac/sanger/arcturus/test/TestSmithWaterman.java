...
public class TestSmithWaterman {
	...
	public static void main(String args[]) {
		...
		ScoringMatrix defaultMatrix = new ScoringMatrix(matchScore,
				mismatchScore, gapinitScore, gapextScore);

		report("Starting Smith-Waterman calculation.");

		SmithWatermanEntry[][] sw = SmithWaterman.calculateMatrix(seqA, startA,
				endA, seqB, startB, endB, defaultMatrix);
		...
		System.out.println("Maximum score (" + maxScore + ") found at row "
		...
		while (score > 0 && col > 0 && row > 0) {
			...
			switch (direction) {
				case SmithWatermanEntry.UNDEFINED:
					dir = '?';
					break;
			...
			System.out.println(score + " " + row + " " + baseA + " " + col
					+ " " + baseB + " " + dir);
	...
	public static void report(String message) {
		...
		System.err.println("Memory (kb): (free/total) " + runtime.freeMemory()
