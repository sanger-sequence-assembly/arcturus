...
public class Gap4BayesianConsensus implements ConsensusAlgorithm {
	...
	if (debugps != null) {
	    debugps.println();
	    debugps.println("Gap4BayesianConsensus::addBase(" + base + ", " + quality + ", " + strand + ", " + chemistry + ")");
	}

	if (strand == ConsensusAlgorithm.UNKNOWN || chemistry == ConsensusAlgorithm.UNKNOWN)
	    return false;
	...
	// iStrandAndChemistry is the second index into the arrays "qhighest" and "qcount"
	//
	...
	// FROM qual.c:
	//
	// qual == 1 implies less likely to be the called base. We map this to 2, as
	// this is undesirable.
	...
	// FROM qual.c:
	//
	// qual == 0 implies "ignore this base". We do this by changing the type to dash,
	// to force even spread of probability. Otherwise, we'd actually be negatively
	...
	if (debugps != null) {
	    debugps.println("  qhighest[" + iBase + "][" + iStrandAndChemistry + "] = " + qhighest[iBase][iStrandAndChemistry]);
	...
	for (int k = 0; k < 4; k++) {
	    if (qhighest[5][k] > 0) {
		double tmp = (double)(qhighest[5][k] + dependent_table[Math.min(qcount[5][k], 10)]);
		...
		if (debugps != null) {
		    debugps.println();
		    debugps.println("  qhighest[5][" + k + "] = " + qhighest[5][k]);
		    debugps.println("  qcount[5][" + k + "] = " + qcount[5][k]);
		    debugps.println("  score = " + tmp);
		    debugps.println("  Sharing probability " + decimal.format(sharedprob) + " amongst " + nbase_types + " base types");
		}
	...
	for (int j = 0; j < nbase_types; j++) {
	    for (int k = 0; k < 4; k++) {
		if (qhighest[j][k] > 0) {
		    double tmp = (double)(qhighest[j][k] + dependent_table[Math.min(qcount[j][k], 10)]);
		    double prob = Math.exp(-log10 * tmp/10.0);
		    ...
		    if (debugps != null) {
			debugps.println();
			debugps.println("  qhighest[" + j + "][" + k + "] = " + qhighest[j][k]);
			debugps.println("  qcount[" + j + "][" + k + "] = " + qcount[j][k]);
			debugps.println("  score = " + tmp);
			debugps.println("  Assigning probability " + decimal.format(prob) + " to base " + j);
			debugps.println("  Sharing probability " + decimal.format(sharedprob) + " amongst other bases");
		    }
	...
	if (debugps != null) {
		...
		break;
	    default:
		debugps.print("There are " + nEvents + " independent events");
		break;
	    }
	...
	if (nEvents > 0) {
		...
		if (debugps != null) {
		    debugps.println("  Normalised probability and score for base " + j + " = " + 
				    decimal.format(probs[j]) + ", " + scores[j]);
		}
	    }
	}
