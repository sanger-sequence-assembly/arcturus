...
public class ContigCAFWriter {
	...
	public int writeContigAsCAF(int contigid, PrintWriter pw)
		...
		while (rs.next()) {
			...
			while (rs2.next()) {
				...
				if (forward)
					pw.println("Assembled_from " + readname + " " + cstart + " " + cfinish + " " + rstart + " " + rfinish);
				else
					pw.println("Assembled_from " + readname + " " + cfinish + " " + cstart + " " + rfinish + " " + rstart);
			}
	...
	private int writeRead(int readid, int seqid, PrintWriter pw)
		...
		while (rs.next()) {
			...
			pw.println("Seq_vec SVEC " + svleft + " " + svright + " \"" + svname + "\"");
		}
		...
		while (rs.next()) {
			...
			pw.println("Clone_vec CVEC " + cvleft + " " + cvright + " \"" + cvname + "\"");
		}
	...
	private byte[] decodeCompressedData(byte[] compressed, int length) throws DataFormatException {
	...
	public static void main(String[] args) {
		try {
			...
			for (int i = 0; i < args.length; i++) {
				int contigid = Integer.parseInt(args[i]);
				
				if (contigid > 0) {
					File file = new File(organism + contigid + ".caf");
