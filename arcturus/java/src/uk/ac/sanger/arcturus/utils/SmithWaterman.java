...
public class SmithWaterman {
	public static SmithWatermanEntry[][] calculateMatrix(byte[] sequenceA,
			int starta, int enda, byte[] sequenceB, int startb, int endb,
			ScoringMatrix smat) {
		...
		for (int row = 1; row < nrows; row++) {
			char baseA = Character.toUpperCase((char) sequenceA[starta + row
					- 2]);

			for (int col = 1; col < ncols; col++) {
				char baseB = Character.toUpperCase((char) sequenceB[startb
						+ col - 2]);
				...
				if (diagonal > 0 || maxGapScore > 0) {
					if (diagonal >= maxGapScore) {
						sw[row][col].setScoreAndDirection(diagonal,
								SmithWatermanEntry.DIAGONAL);
					} else {
						if (up > left)
							sw[row][col].setScoreAndDirection(up,
									SmithWatermanEntry.UP);
						else
							sw[row][col].setScoreAndDirection(left,
									SmithWatermanEntry.LEFT);
					}
				} else {
					sw[row][col].setScoreAndDirection(0,
